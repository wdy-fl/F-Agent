    ...
    schema={
        ...
        "function": {
            ...
            "description": (
                ...
                "创建技能时使用清晰的名称和描述，内容应具体、可执行。"
                "创建/删除前必须征求用户确认；创建前应询问用户希望将技能归入哪个分类，"
                "例如 dev、tools、testing；用户不指定时使用 uncategorized，"
                "并在 SKILL.md 的 frontmatter 中填写 category 字段。"
                "使用技能时发现内容过时、不完整或错误，用 edit 修正。"
                "技能变更后需重启会话才能生效。"
            ),
            "parameters": {
                "type": "object",
