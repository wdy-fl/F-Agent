...
def set_managers(memory_manager: "MemoryManager | None" = None) -> None:
    """注入记忆管理器引用，在 AgentLoop 初始化时调用"""
    _memory_manager_var.set(memory_manager)
...
def handle_memory(args: dict) -> str:
    ...
    if action == "search":
        ...
        memory_manager = _memory_manager_var.get()
        if not memory_manager:
            return json.dumps({"error": "Memory manager not available"}, ensure_ascii=False)
        results = memory_manager.session_db.search_messages(query, limit=limit)
        ...
        if not memory_manager:
            return json.dumps({"error": "Memory manager not available"}, ensure_ascii=False)
        content = memory_manager.get_user_profile()
        ...
        if not memory_manager:
            return json.dumps({"error": "Memory manager not available"}, ensure_ascii=False)
        new_profile = memory_manager.update_profile(observations)
        ...
        if not memory_manager:
            return json.dumps({"error": "Memory manager not available"}, ensure_ascii=False)
        memory_manager.append_to_memory(content)
        ...
        if not memory_manager:
            return json.dumps({"error": "Memory manager not available"}, ensure_ascii=False)
        content = memory_manager.get_soul()
        return json.dumps({"content": content}, ensure_ascii=False)
        ...
        if not memory_manager:
            return json.dumps({"error": "Memory manager not available"}, ensure_ascii=False)
        new_content = memory_manager.update_soul(content)
        ...
        if not memory_manager:
            return json.dumps({"error": "Memory manager not available"}, ensure_ascii=False)
        content = memory_manager.get_agent()
        return json.dumps({"content": content}, ensure_ascii=False)
        ...
        if not memory_manager:
            return json.dumps({"error": "Memory manager not available"}, ensure_ascii=False)
        new_content = memory_manager.update_agent(content)
        return json.dumps({"status": "updated", "length": len(new_content)}, ensure_ascii=False)
