...
F-Agent（阿福）是一个面向个人研发场景的本地 CLI 智能助手。当前版本聚焦 Agent 核心闭环：对话、工具调用、会话持久化、历史召回、记忆同步、技能复用和上下文压缩。
...
| 安全 | 审批优先 | 对终端高风险命令默认交互确认，降低本地破坏风险 |
...
- CLI 层负责用户交互和审批回调，不把提示词构建、工具调度等逻辑塞进界面代码。
...
CLI 和 AgentLoop 共享同一个 `SessionDB` 实例。CLI 同时注册命令审批回调，在终端工具遇到危险命令时展示 rich Panel 并等待用户选择。
...
审批状态按会话记录，`approval.mode = off` 时可关闭危险命令审批，但 hardline 规则仍由检测逻辑优先判断。
